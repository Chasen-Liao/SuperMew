...
PARENT_CHUNK_STORE_PATH = Path(__file__).resolve().parent.parent.parent / "data" / "eval_parent_chunks_2018.json"
CHUNK_SIZE = 1024
CHUNK_OVERLAP = 128
...
def load_data(path: Path) -> list[dict]:
    """加载 CMRC 2018 JSON（标准 QA 格式：question + context + answers）"""
    if not path.exists():
...
def build_chunks_from_context(context: str, question_id: str) -> list[dict]:
    ...
    for l1_idx, l1_doc in enumerate(splitter_l1.create_documents([context])):
        ...
        chunks.append({
            "text": l1_text, "filename": question_id, "file_type": "CMRC2018",
        ...
        for l2_idx, l2_doc in enumerate(splitter_l2.create_documents([l1_text])):
            ...
            chunks.append({
                "text": l2_text, "filename": question_id, "file_type": "CMRC2018",
            ...
            for l3_idx, l3_doc in enumerate(splitter_l3.create_documents([l2_text])):
                ...
                if not l3_text:
                    continue
                chunks.append({
                    "text": l3_text, "filename": question_id, "file_type": "CMRC2018",
...
def run():
    ...
    print(f"[CMRC 2018] {len(records)} 条记录")

    print("[CMRC 2018] 构建 ground truth...")
    ...
    print(f"[CMRC 2018] ground truth 已保存: {gt_path}")
    ...
    print(f"[CMRC 2018] 总 chunks: {len(all_chunks)}")

    # Milvus
    print(f"[CMRC 2018] 重建 collection: {EVAL_COLLECTION}")
    ...
    print(f"[CMRC 2018] 索引完成: {len(all_chunks)} chunks")
