"""HotpotQA 多跳问答 - 检索评测脚本（含 HyDE 实验）"""
import json
import sys
import csv
import logging
...
from config import LEAF_RETRIEVE_LEVEL, API_KEY, BASE_URL
...
def generate_hypothetical_document(query: str, model) -> str:
    ...
    try:
        ...
        logging.warning(f"[HyDE] 假设文档生成失败: {e}")
        return ""
...
def run():
    ...
    if not gt_path.exists():
        raise FileNotFoundError(f"ground truth 未找到，请先运行 eval_indexer_hotpot.py: {gt_path}")
    with open(gt_path, "r", encoding="utf-8") as f:
        gt_map: dict = json.load(f)

    mm = MilvusManager(collection_name=EVAL_COLLECTION)
    if not mm.has_collection():
        raise RuntimeError(f"collection 不存在: {EVAL_COLLECTION}，请先运行 eval_indexer_hotpot.py")
    ...
    print(f"\n[HotpotQA] 开始 Baseline 评测，共 {len(records)} 条数据")
    for i, rec in enumerate(records, 1):
        qid = rec.get("_id", rec.get("id", ""))
        question = rec.get("question", "")
        gt_ids = set(gt_map.get(qid, []))
    ...
    print("\n" + "=" * 60)
    print("HotpotQA Baseline 评测结果汇总")
    print("=" * 60)
    csv_path = RESULTS_DIR / f"hotpotqa_{ts}_metrics.csv"
    ...
    # 写入 HyDE 独立 CSV
    hyde_csv_path = RESULTS_DIR / f"hotpotqa_hyde_{ts}_metrics.csv"
    with open(hyde_csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=fields + ["hyde_success_rate"])
        writer.writeheader()
        if hyde_results:
            ...
            writer.writerow(row)
            print(f"  hyde: P@5={row['precision']} R@5={row['recall']} "
                  f"MRR={row['mrr']} NDCG@5={row['ndcg']} (n={row['query_count']})")
